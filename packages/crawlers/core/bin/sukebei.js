...
dist.sortMagnets = sortMagnets;

const program = new commander.exports.Command();
program.name('sukebei').argument('<code>', 'search key.').helpOption('-h, --help', `
    sukebei dvaj-532
    sukebei fsdss-288

    more: https://sukebei.nyaa.si/
    `).action(async code => {
  const magnets = await getMagnets_1(code);
  await dist$1.usePlaywright().then(({
    close
  }) => close());
  const output = magnets.map(v => {
